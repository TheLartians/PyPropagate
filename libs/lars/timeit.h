...
namespace lars {
  ...
  template <class Duration> std::string duration_to_string(const Duration &dur){
    ...
    if(h.count() > 1) return std::to_string(h.count()) + "h";
    if(m.count() > 1) return std::to_string(m.count()) + "m";
    if(s.count() > 1) return std::to_string(s.count()) + "s";
    if(ms.count() > 1) return std::to_string(ms.count()) + "ms";
  ...
  template<size_t N = 1000, bool calculate_std = false,  class F,typename ... Args> __attribute__ ((noinline)) void timeit(const std::string &name,F f,Args & ... args){
    ...
    if(!calculate_std){
      ...
      double std = sqrt( squared/N - total/N  * total/N  ) ;
