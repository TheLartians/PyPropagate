...
template <class Shape,size_t N = Shape::size()> struct NDArrayCalculator{
  using NextProd = typename NDArrayCalculator<Shape,N-1>::NextProd::template SliceType<1,Shape::size()>::template push_back_static_type<1>;
  using Prod = typename NextProd::template mul_result< typename NDArrayCalculator<Shape,N-1>::Prod  >;
  using Stride = typename Prod::template SliceType<1,Shape::size()>::template push_back_static_type<1>;
...
template <class T> struct BorrowedData{
  ...
  template <class T,typename TShape,typename TStride,typename TOffset,typename TData,typename TCreator> class NDArrayBase{
public:
...
public:
  ...
  NDArrayBase(Shape shape,Stride stride,Offset offset,Data && data):_shape(shape),_stride(stride),_offset(offset),_data(std::forward<Data>(data)){ }
  NDArrayBase(Shape shape,Stride stride,Offset offset,const Data & data):_shape(shape),_stride(stride),_offset(offset),_data(data){ }

  template <typename ... DataArgs> NDArrayBase(Shape shape,Stride stride,Offset offset,DataArgs ... data_args):_shape(shape),_stride(stride),_offset(offset),_data(data_args...){ }
  ...
  template <typename Pos,typename Sha,typename Ste> using SliceType = NDArray<T,Sha, typename Stride::template mul_result<Ste>, DynamicIndex, BorrowedData<T> >;
  template <typename Pos,typename Sha,typename Ste> using ConstSliceType = NDArray<const T,Sha, typename Stride::template mul_result<Ste>, DynamicIndex, BorrowedData<const T> >;
  
  template <typename Pos,typename Sha,typename Ste = IndexTupleRepeat<1, Shape::size()>> SliceType<Pos,Sha,Ste> slice(Pos pos,Sha shape,Ste step = IndexTupleRepeat<1, Shape::size()>()){
#ifndef  NDEBUG
    ...
    return SliceType<Pos,Sha,Ste>(shape,stride()*step,offset,data());
  }
  
  template <typename Pos,typename Sha,typename Ste = IndexTupleRepeat<1, Shape::size()>> ConstSliceType<Pos,Sha,Ste> slice(Pos pos,Sha shape,Ste step = IndexTupleRepeat<1, Shape::size()>())const{
#ifndef  NDEBUG
    check_index check(_shape);
    (pos + step*(shape - IndexTupleRepeat<1, Shape::size()>())).apply_template(check);
#endif
    size_t offset = get_data_index(pos);
    return ConstSliceType<Pos,Sha,Ste>(shape,stride()*step,offset,data());
  }

  
  using ElementType = typename std::conditional<( Shape::size() > 1), NDArray<T,typename Shape::template SliceType<1,Shape::size()>,typename Stride::template SliceType<1,Shape::size()>, DynamicIndex, BorrowedData<T> >, T & >::type;
  
  using ConstElementType = typename std::conditional<( Shape::size() > 1), NDArray<const T,typename Shape::template SliceType<1,Shape::size()>,typename Stride::template SliceType<1,Shape::size()>, DynamicIndex, BorrowedData<const T> >, const T & >::type;
  ...
  struct const_iterator:public std::iterator<std::input_iterator_tag, std::remove_reference<ConstElementType> >{
    ...
    ConstElementType operator*()const{ return parent[current_index]; }
  ...
  template <typename D = void> disable_if_one_dimensional<const ConstElementType,D> operator[](size_t i)const{
  ...
  Transposed transpose(){
    return Transposed(reverse(_shape),reverse(_stride),_offset,data());
  }
  ...
  // Operators
    ...
    Super & operator/=(const Scalar &s){
      this->for_all_indices([&](Index idx){ (*this) /= s; });
      return *(Super *)this;
    }
...
template <class T,typename Shape,typename Stride,typename Offset,typename Data, typename Creator> std::ostream & operator<<(std::ostream &stream,const NDArrayBase<T, Shape, Stride, Offset, Data, Creator> & array){
  stream << '[';
...
public:
  ...
  template <typename S> void resize(S shape){ Base::_shape.set(shape); Base::_stride.set(NDArrayCalculator<Shape>::stride(shape)); Base::_data.resize(NDArrayCalculator<Shape>::prod(shape).template get<0>()); }
  ...
  public:
    ...
    template <typename oT,typename oShape,typename oStride,typename oOffset,typename oData, typename oCreator> StackNDArray(const NDArrayBase<oT, oShape, oStride, oOffset, oData, oCreator> & other):Base(other.shape(),NDArrayCalculator<Shape>::stride(other.shape()),StaticIndex<0>()){
      Base::operator=(other);
    }

    // template <typename ... Size> StackNDArray(size_t first, Size ... rest):Base(make_dynamic_index_tuple(first,rest...),NDArrayCalculator<Shape>::stride(make_dynamic_index_tuple(first,rest...)),StaticIndex<0>()){}
    
    template <typename S> void resize(S shape){ Base::_shape.set(shape); }
    template <typename ... Args> void resize(size_t first,Args ... rest){ resize(Shape(first,rest...)); }

  };
  
  //template <class T,size_t D, template<class,class> class Array = HeapNDArray> using NDArray = Array<T, DynamicIndexTuple<D>>;
  // template <class T,size_t D,class C = BasicNDArrayCreator> using MappedNDArray = NDArrayBase<T, DynamicIndexTuple<D>, DynamicIndexTuple<D>, DynamicIndex, BorrowedData<T>,C>;
